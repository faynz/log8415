...
\paragraph{}
We had the opportunity to explore the MapReduce paradigm. We successfully compared the performance of the algorithm on Linux, Hadoop and Spark with different experiments. At first, we compared Linux and Hadoop in a simple WordCount program to observe the difference in performance between the two. And then, we compared Hadoop vs Spark on AWS, Amazon’s cloud computing platform, by creating an EC2 instance.

\section{Performance comparison of Hadoop vs Linux} \label{sec:HadoopVsLinux}

\begin{center}
  The result shows that our local machine Linux outperformed Hadoop.
\end{center}
