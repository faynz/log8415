...
\paragraph{} We began to experiment with the assumption that Spark would outperform Hadoop due to the fact Apache Spark processes the data in-memory instead of disk after performing a MapReduce. The previous table did not show us what we expected. In fact, we believed Spark would be at least 2x faster than Hadoop, in this case where we aren't dealing with Big Data. When dealing with Big Data, Spark's performance could spike up to 3x faster according to Goran Jevtic \cite{1}.
